...
本レポートでは、Google が公開している OSS である **DPSynth** を評価対象とする。DPSynth は公開されて間もないライブラリであり、第三者による実証的な評価例がまだ少ない。そこで、実データに近い公開データ（UCI Adult Income）を用いて、その有用性・経験的プライバシー・実装上の制約を早期に検証することを試みる。本レポートの貢献は以下の三点に整理される。
...
下表は、機構比較（`ε=1.0`）と、MST について ε を 4 水準に変えた比較の結果をまとめたものである。各列は左から、平均 1-way TVD（小さいほど分布が一致）、相関誤差（小さいほど数値相関を保持）、TSTR の AUC・正解率・F1（大きいほど下流タスクに有用）、生成時間である。最下行の TRTR は実データ学習による上限の参照点を示す。
...
- **ε・ビン数に対する感度分析**: プライバシー予算と数値ビン数を複数水準に変えた体系的な比較により、トレードオフをより精緻に特徴づける。
