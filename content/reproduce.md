...
環境/シード間で 0.226 / 0.007 / 0.404 と大きく動いた例がある — Issue #15）。
本リポジトリは**固定環境を WSL2 + `requirements.txt` のピン留め依存**と定め、
レポートの数値・図は **すべてこの固定環境の単一の実行系列**（`run_all.sh` 一括実行）で再生成して統一している（Issue #17）。
本レポートの数値を確定した固定環境は次のとおり:

- **OS / Python**: WSL2（Ubuntu 24.04）／ Python 3.12.3
- **主要依存**: `requirements.txt` でピン留め（`jax==0.7.1` / `jaxlib==0.7.1` / `numpy==1.26.4` /
  `scikit-learn==1.8.0` / `scipy==1.17.1` / `pandas==3.0.3` / `matplotlib==3.10.9`、`mbi` はコミットハッシュ固定）
- **upstream dpsynth**: `c17e714818c3e62375246f0e1b59e486244a469d`（`setup_env.sh` 実行後に `src_commit.txt` へ記録）。
  以後は `DPSYNTH_REF=c17e71481… bash scripts/setup_env.sh` で固定する。
