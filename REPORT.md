...
- **下流タスク性能（TSTR）**: **TSTR（Train on Synthetic, Test on Real）** は、合成データで予測モデルを学習し、実データで評価する手法である。本実験では合成データで `income>50K` の二値分類器（勾配ブースティング）を学習し、合成元に**含まれない**実データ 8,000 行で評価した。比較の参照値として、実データで学習・実データで評価する **TRTR（Train on Real, Test on Real）** を併記する。TRTR は本実験での到達目標を示す**参照上限**であり、理論的な上界ではなく経験的な参照値である点に留意する。
- **経験的プライバシー（MIA）**: **MIA（Membership Inference Attack, メンバーシップ推論攻撃）** は、あるレコードが学習（合成元）に含まれていたか否かを推定する攻撃である [\[15\]](#ref15)。本実験では、各レコードの合成データへの最近傍距離を手がかりにメンバー／非メンバーを判別する距離ベースの MIA を行い、判別性能を **ROC-AUC** で測る。AUC ≈ 0.5 はメンバーシップが漏れていない（漏洩なし）ことを意味する。
