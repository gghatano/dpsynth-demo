...
- **プライバシー予算 ε を変えた比較（MST、AIM は同条件を実装）**: 機構を MST に固定し、ε のみを `0.5 / 1.0 / 2.0 / 10.0` の 4 水準に変えて生成する（`δ=1e-5` 固定）。同じ ε 水準の AIM スイープも同条件で実装済みだが、本実行環境では一部の ε で JIT コンパイルがメモリ確保に失敗し実測が未取得（pending）である。AIM スイープの設計・想定論点・課題は [AIM の解説 §7](method-aim.html) に分離している。
...
**図2: プライバシー–有用性トレードオフ（MST）** — 横軸が ε、縦軸が平均 1-way TVD。ε を大きくする（プライバシーを緩める）ほど平均 TVD がおおむね低下する傾向を示す。ε=2.0 の非単調性は単一シードのばらつきによる。なお評価スクリプトは AIM 系列も同じ図に重ねるよう実装済みで、AIM スイープの実測が得られれば本図に AIM の曲線が追加される（現状は実測 pending のため MST のみ）。AIM の ε 依存に関する想定論点・課題は [AIM の解説 §7](method-aim.html) を参照。
