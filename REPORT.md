...
> 📑 **凡例: 記載の区別**(各節の冒頭にどちらかを明記します)
> - 📘 **ドキュメント記載** … google/dpsynth の README・公式ドキュメント・論文に基づく**事実**(出典は末尾の付録C)。
> - 🔎 **考察** … 本デモの**実測・分析・推定・意見**(一次情報)。ドキュメントには無い、筆者の解釈・評価。
> - 📘+🔎 **混在** … 事実の引用と考察が混ざる節(段落単位で「🔎 考察:」と明示)。

---

## 0. エグゼクティブサマリ

> 📘+🔎 **混在**（全体の要約。詳細な区分は各節を参照）
...
> 📘 **ドキュメント記載**（§1.1〜1.4。出典は付録C）。ただし **§1.5 は 🔎 考察**。
...
> 📘 **ドキュメント記載**（公式ドキュメント・README・各論文に基づく。出典は付録C）。
...
> 📘 **ドキュメント記載**（API シグネチャ・YAML 規則・CLI は公式ドキュメント/ソース準拠。出典は付録C）。
...
> 📘+🔎 **混在**。評価・優劣の判断は 🔎 考察。「非公式・サポート外」「依存・対応 Python」等の**事実は 📘 ドキュメント記載**。
