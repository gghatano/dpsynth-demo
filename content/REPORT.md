...
いずれの機構も、推定エンジンとして **Private-PGM**（[`mbi`](https://github.com/ryan112358/mbi)、測定済みのマージナルと整合する同時分布をグラフィカルモデル＝マルコフ確率場として推定する手法）を共有する [\[10\]](#ref10)。すなわち各機構の違いは「**どのマージナルを、どの予算配分で測定するか**」という戦略の違いに帰着し、測定値から同時分布を推定する土台は 3 機構で共通である。共有エンジンの仕組み（mirror descent による推定、junction tree とモデルサイズ、ウォームスタート、各機構が何を渡すか）は本文には含めず、[Private-PGM の解説ページ](method-pgm.html)に分離した。

> 📘 **機構選定に関する注記**: In-Memory API `dpsynth.generate()` の**デフォルト機構は MST** である（`discrete_config` 引数の既定値が `MSTConfig()`）。DPSynth はデータ特性に応じて機構を自動選定する機能を持たないため、**どの機構を採用するかはユーザ自身の責任で判断する必要がある**。選定の考え方は [google/dpsynth 公式ドキュメント](https://github.com/google/dpsynth)に断片的な記載があるのみのため、本サイトでは補足として各機構の解説ページ（[MST](method-mst.html)・[AIM](method-aim.html)・[INDEPENDENT](method-independent.html)）と、3 機構が共有する推定エンジンの[Private-PGM 解説](method-pgm.html)、本レポートの実験結果に基づく[手法選定ガイド](method-selection.html)を用意した。
...
- **プライバシー予算 ε を変えた比較（MST・AIM・INDEPENDENT）**: ε のみを `0.5 / 1.0 / 2.0 / 10.0` の 4 水準に変えて生成する（`δ=1e-5` 固定）。§5.1 の主表は MST 単一シードの ε スイープを示すが、3 機構すべての ε 依存を**複数シードの平均±標準偏差**で評価した結果を [追加実験](experiments.html)（実験E）と [AIM の解説 §7](method-aim.html) にまとめている。単一シードでは run-to-run 分散が ε トレンドを覆う（特に AIM）ため、ε 依存の判断はマルチシードに基づく。
...
|---|---|---|---|---|---|---|
| **MST** (ε=1.0) | 0.101 | 0.089 | 0.663 | 0.732 | 0.407 | 約 10 秒 |
| **AIM** (ε=1.0) | 0.070 | 0.404 † | **0.758** | 0.743 | 0.479 | 約 70 秒 |
| **INDEPENDENT** (ε=1.0) | 0.060 | 0.082 | 0.443 | 0.760 | 0.003 | 約 6 秒 |
| MST (ε=0.5) | 0.066 | 0.066 | 0.665 | 0.726 | 0.460 | 約 9 秒 |
| MST (ε=2.0) | 0.054 | 0.040 | 0.688 | 0.738 | 0.434 | 約 9 秒 |
| MST (ε=10.0) | 0.059 | 0.072 | 0.683 | 0.745 | 0.409 | 約 7 秒 |
| *TRTR 参照上限(実データ学習)* | *—* | *—* | *0.893* | *0.841* | *0.625* | *—* |

本表は単一シード（`seed=42`）・WSL 固定環境での代表的な 1 実行の結果である。**1-way TVD と相関誤差は run-to-run 分散が大きく、単一シードの大小をそのまま機構の優劣と読んではならない**（複数シードでの評価は [追加実験](experiments.html) 実験B・実験E）。太字（TSTR AUC の AIM）は、複数シードでも安定して確認できる唯一の機構間順序を示す。

> † **AIM ε=1.0 の相関誤差 0.404 は単一シードの外れ値**である。同じ条件を 10 シードで測ると AIM の相関誤差は **0.022 ± 0.011**（[実験E](experiments.html)）と実際には小さく、単一シードの 0.404 は典型値を大きく外す。AIM の ε=1 相関誤差は run-to-run でも振れやすく（別環境の単一実行では 0.226 や 0.007、別実行の 5 シードでは 0.104±0.163 も観測）、点推定では評価できない。§6 の解釈はマルチシードに基づく。

本表から、本実験条件において以下の点が**事実として**読み取れる（定性的判断はいずれも複数シードで裏づけたもの）。

- 平均 1-way TVD は 3 機構とも 0.06〜0.10 の範囲にあり、単一シードでは大小が前後する。複数シードでも 3 機構はほぼ同程度（ε=1 で MST 0.090 / AIM 0.087 / INDEPENDENT 0.088、[実験E](experiments.html)）で、この指標だけでは機構間の差はほとんど判別できない。
- TSTR AUC は AIM（0.758）> MST（0.663）> INDEPENDENT（0.443）の順で、INDEPENDENT のみ大きく劣る。INDEPENDENT は TSTR F1 が約 0 であり、ほぼ全件を `<=50K` と予測している。この下流性能の順序（AIM ≳ MST > INDEPENDENT）は複数シードでも再現する（[追加実験](experiments.html) 実験B）。
- MST の平均 1-way TVD は ε の増加に伴っておおむね改善するが、単一シードでは非単調に見える（確率的ばらつきによる）。複数シードでは ε=0.5 → 10 で 0.089 → 0.061 とおおむね低下する（高 ε ほど改善し標準偏差も縮む。[実験E](experiments.html)）。
...
**図2: プライバシー–有用性トレードオフ（単一シード）** — 横軸が ε、縦軸が平均 1-way TVD。ε を大きくする（プライバシーを緩める）ほど平均 TVD がおおむね低下する傾向を示す。単一シードのため曲線には確率的なばらつきが残る。**3 機構すべての ε 依存を複数シードの平均±標準偏差（エラーバー）で評価した図は [追加実験](experiments.html) 実験E（`expE_eps_multiseed.png`）にあり**、ε トレンドの判断はそちらに基づく。AIM の ε 依存の詳細な所見は [AIM の解説 §7](method-aim.html) を参照。
...
- 非 DP のコピーは AUC 0.894 であり、攻撃自体は機能している。
- DP 合成データは機構・ε によらず AUC ≈ 0.51 であり、本攻撃ではメンバーシップの手がかりがほぼ検出されなかった。
- ε による AUC の差はほとんど見られない（0.50〜0.52）。
...
本実験条件では、列間依存を扱う MST／AIM が、独立仮定の INDEPENDENT より下流タスク性能で優位だった。この差は、平均 1-way TVD ではほとんど現れない一方、TSTR AUC で明確に現れた（第5.1節）。INDEPENDENT は 1-way（各列単独）の分布は良好に再現するが、全属性を独立と仮定するため、特徴量と目的変数 `income` の間の依存関係を一切保持しない。この差が、相関の有無に敏感な下流予測タスクで顕在化したと解釈できる。**検証対象である DPSynth について本実験が示すのは、「1-way 分布の一致は必ずしも下流の有用性を意味しない」「列間依存をモデル化する機構が下流タスクの有用性に寄与する」という点**である。

機構間の差（AIM がやや上回る理由、MST が苦手とするペアの崩れ方など）は、各機構のアルゴリズム由来の性質であり、本レポートではなく個別ページに整理した（[AIM の解説](method-aim.html)・[MST の解説](method-mst.html)。3 機構が共有する推定エンジンは [Private-PGM の解説](method-pgm.html)）。本実験の範囲では、AIM が MST をやや上回ったものの計算コストは約 70 秒（MST の約 7 倍。高 ε ではさらに重く、ε=10 で約 420 秒）であり、両者の下流タスク性能の優劣はシード次第で逆転しうる程度の差にとどまった（[追加実験](experiments.html) 実験B）。一般論として AIM が常に優れると断定はできない。
