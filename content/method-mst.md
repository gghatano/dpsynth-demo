...
- **機構比較（ε=1.0、[メインレポート §5.1](index.html)）**: 平均 1-way TVD 0.101、相関誤差 0.089、
  TSTR AUC 0.663、生成時間**約 10 秒**（単一シード）。
- **複数シード（[追加実験B](experiments.html)）**: TSTR AUC 0.651 ± 0.106（10 シード）。
  AIM（0.709 ± 0.051）をやや下回るが標準偏差が重なり、優劣はシード次第で逆転しうる（AIM ≳ MST）。
- **2-way 忠実度（[追加実験C](experiments.html)）**: ペアによって明暗が分かれる。
  `education×income` は 0.024 と良好だが、木から外れた `marital-status×income`（0.359）・
  `occupation×income`（0.282）は大きく崩れ、**INDEPENDENT より悪い**ケースもあった。
- **ε スイープ（複数シード・[追加実験E](experiments.html)）**: ε を 0.5 → 10 に上げると平均 1-way TVD は
  0.089 → 0.061 へおおむね改善する（ε=0.5→1.0 は分散内の微増、それ以降は単調低下。標準偏差も ε とともに縮む）。
