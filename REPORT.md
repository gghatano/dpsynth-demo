# DPSynth を用いた差分プライバシー付き合成テーブルデータ生成の実証評価
...
> 📑 **凡例: 事実と考察の区別**
> 本レポートでは、出典に基づく事実と筆者の分析・推定を区別して記述する。
> - 出典タグ **[n]**（例 [\[3\]](#ref3)）… google/dpsynth のドキュメント・論文・ソースコードに基づく**事実**。クリックで第10章「参考文献」の該当出典へ移動する。
> - 🔎 **考察** … 出典タグの**無い**記述で、本実験で得た**実測・分析・推定**(一次情報)。章・節が丸ごと考察に当たる場合は見出し直下に 🔎 を付す。
>
> なお、本レポートはエンジニアリング上の手順（環境構築・API 利用・再現コマンド・実装上の不具合対応）を本文から分離し、別ページに収めている。該当箇所では都度リンクを示す。

---

## 1. アブストラクト

本レポートは、差分プライバシー（Differential Privacy, DP）付きの合成テーブルデータ生成ライブラリ **DPSynth**（google/dpsynth）を対象とした実証評価である。公開データセット **UCI Adult Income**（48,842 行）から抽出した 20,000 行を合成元とし、In-Memory DataFrame API を用いて複数の生成機構（MST・AIM・INDEPENDENT）を比較した。評価観点は、(1) 周辺分布の再現性、(2) 下流の機械学習タスク（収入予測）の性能、(3) メンバーシップ推論攻撃（MIA）による経験的プライバシーの三点である。本実験条件では、列間の依存関係を扱う MST／AIM が列間相関を一定程度保持し、各属性を独立に扱う INDEPENDENT より下流タスク性能が高かった。一方で、数値列の離散化に伴う分布再現性の低下、単一フラット表を前提とする適用範囲の制約、研究段階に由来する実装成熟度の課題が確認された。これらの結果は単一データセット・特定の依存バージョンに基づくものであり、一般化には追加検証を要する。
...
**差分プライバシー（Differential Privacy, DP）** は、ある 1 個人のレコードを加えても除いても、アルゴリズムの出力分布がほとんど変化しないことを数学的に保証する枠組みである。この保証の強さはプライバシー予算 `(ε, δ)` で制御される。**合成データ（synthetic data）** とは、元データそのものではなく、元データの統計的性質を模した人工的に生成されたデータを指す。**差分プライバシー付き合成データ生成**は、DP を満たしつつ元データの分布・相関を保った合成データを生成することで、上記の課題に対する一つの選択肢を与える。生成された合成データは特定個人の復元が困難であるため、比較的自由に共有・分析できる可能性がある。

本レポートでは、Google が公開している OSS である **DPSynth** を対象に、実データに近い公開データ（UCI Adult Income）を用いて、その実用性・有用性・制約を実験的に評価する。本レポートの貢献は以下の三点に整理される。
...
3. 結果に基づき、DPSynth の有用性・経験的プライバシー・実装上の制約を整理し、適用が見込める用途と適しにくい用途を考察した（第6〜7章）。
...
DPSynth は、機微なテーブルデータから、差分プライバシー `(ε, δ)` を保証しつつ、元データの統計的性質（分布・列間相関）を保った合成データを生成することを目的とする OSS ライブラリである [\[1\]](#ref1)[\[2\]](#ref2)。中核となるのは、**周辺分布（マージナル, marginal）** ——一部の列だけに注目した分布、たとえば 1 列の分布（1-way）や 2 列の同時分布（2-way）——を DP 付きで測定し、それらと整合する同時分布をグラフィカルモデルで推定してサンプリングするという手法である [\[5\]](#ref5)[\[10\]](#ref10)。個々のレコードを記憶するのではなく、ノイズを加えた集計統計量のみからモデルを構築するため、DP が成立する。
...
| 機構 | 概要 | 特徴 |
|---|---|---|
| **AIM** (Adaptive Iterative Mechanism) | ワークロード・予算・データ特性に基づき、低次元のマージナルを**適応的に選択・測定**する MWEM 系手法 [\[8\]](#ref8) | 下流タスクに効く相関を優先的に捉えやすい。計算コストは相対的に高い |
| **MST** (Maximum Spanning Tree) | 指数機構で**ペア相関の最大全域木**を選び、2 変量依存を DP でモデル化する [\[9\]](#ref9) | 高速。多くの場合に妥当なベースラインとなる |
...
いずれの機構も、推定エンジンとして **Private-PGM**（[`mbi`](https://github.com/ryan112358/mbi)、マルコフ確率場をミラー降下で推定する手法）を共有する [\[10\]](#ref10)。すなわち各機構の違いは「**どのマージナルを、どの予算配分で測定するか**」という戦略の違いとして整理できる。
...
本実験は次の問いを扱う。**差分プライバシーという制約の下で、DPSynth は (i) 元データの分布をどの程度再現し、(ii) 下流の分析タスクにどの程度有用なデータを生成し、(iii) 経験的なプライバシー攻撃に対してどの程度頑健か。また、それらは生成機構の選択やプライバシー予算によってどう変化するか。**
...
- **プライバシー予算スイープ（MST）**: `ε = 0.5 / 1.0 / 2.0 / 10.0`（`δ=1e-5` 固定）。
...
本実験では、前掲の問い (i)〜(iii) に対応して以下の指標を用いる。専門用語は初出時に簡潔に定義する。

- **周辺分布の再現性（1-way TVD）**: 各列について、実データと合成データの分布の差を **TVD（Total Variation Distance, 全変動距離）** で測る。0 に近いほど分布が一致する。
- **列間相関の保持**: 数値ペアの Pearson 相関係数の絶対誤差（相関誤差）。属性ペアの同時分布（2-way）の忠実度は [追加実験](experiments.html)（実験C）で別途評価する。
- **下流タスク性能（TSTR）**: **TSTR（Train on Synthetic, Test on Real）** は、合成データで予測モデルを学習し、実データで評価する手法である。本実験では合成データで `income>50K` の二値分類器（勾配ブースティング）を学習し、合成元に**含まれない**実データ 8,000 行で評価した。上限の参照点として、実データで学習・実データで評価する **TRTR（Train on Real, Test on Real）** を併記する。
- **経験的プライバシー（MIA）**: **MIA（Membership Inference Attack, メンバーシップ推論攻撃）** は、あるレコードが学習（合成元）に含まれていたか否かを推定する攻撃である。本実験では、各レコードの合成データへの最近傍距離を手がかりにメンバー／非メンバーを判別する距離ベースの MIA を行い、判別性能を **ROC-AUC** で測る。AUC ≈ 0.5 はメンバーシップが漏れていない（漏洩なし）ことを意味する。
...
### 5.1 機構比較とプライバシー予算スイープ

下表は、機構比較（`ε=1.0`）と MST のプライバシー予算スイープの結果をまとめたものである。各列は左から、平均 1-way TVD（小さいほど分布が一致）、相関誤差（小さいほど数値相関を保持）、TSTR の AUC・正解率・F1（大きいほど下流タスクに有用）、生成時間である。最下行の TRTR は実データ学習による上限の参照点を示す。
...
**図4: 下流タスク性能（TSTR AUC）** — 各機構の TSTR AUC を棒で、TRTR 上限を参照線で示す。列間相関を保持する MST／AIM が INDEPENDENT を上回り、AIM が TRTR 上限に最も接近していることを示す。これは 5.1 の表の TSTR AUC 列を可視化したものである。
...
本実験の結果と DPSynth の前提を踏まえると、以下の用途では有用性が見込める。

- **単表マスターデータの共有**（1 行 = 1 エンティティ。本実験の Adult が該当）。列間相関と型を保った合成版を提供できる。
- **プロトタイピング・分析コードの事前検証**。本番データを使えない環境で、分布感の近いデータを用いて分析・パイプラインを試作・検証する。
- **データ構造や分布感の共有**。スキーマと統計的性質を保ったサンプルを、再識別リスクを抑えつつ提示する。

### 6.3 適しにくい用途

一方、以下の用途は DPSynth の前提（単一フラット表・1 行 = 1 プライバシー単位）から外れるため、適用が難しいか、保持される構造が変わる。

- **1 個人が複数行を持つトランザクションデータ**。各行の属性分布は保てても、エンティティ別の系列・件数（カーディナリティ）は保たれない。さらに「1 人 = 多数行」では行レベル DP の前提が崩れ、保証の単位が実質的に行（トランザクション）になり、個人単位のプライバシーが弱まる。
- **複数テーブル（マスター–トランザクション）**。外部キー・参照整合性・表間相関は扱えず、リレーショナル合成器の領域となる。
- **時系列・グラフ**。順序・自己相関・ネットワーク構造は対象外である。
- **厳密な個票再現が求められる用途**。DP 合成は集計統計量からの生成であり、個々のレコードを忠実に再現するものではない。

実務でデータ形式を見極める際は、まず自分のデータが上記のどれに当たるかを確認することが有効である。

### 6.4 差分プライバシーの保証と経験的攻撃評価の関係
...
以上を踏まえると、DPSynth を実務に導入する際は、「生成できるか」だけでなく、**有用性評価・安全性評価・利用目的・公開範囲をセットで設計する**ことが望ましい。具体的には、対象タスクに即した有用性指標（本実験の TSTR に相当するもの）での検証、想定脅威に応じた経験的安全性評価、プライバシー予算の設計、および公開範囲に応じたリスク受容の判断を、一体として行う必要がある。

---

## 7. 残課題

研究上の課題と、実装・運用上の課題を分けて整理する。

### 7.1 研究上の残課題

- **複数データセットでの評価**: 本実験は単一データセットに限られる。分布特性の異なる複数データでの再評価が必要である。
- **ε・ビン数に対する感度分析**: プライバシー予算と数値ビン数の体系的なスイープにより、トレードオフをより精緻に特徴づける。
- **実業務データに近いデータでの評価**: 公開データではなく、実務に近い分布・カーディナリティを持つデータでの検証。
- **MIA 以外の攻撃評価**: シャドウモデル型 MIA や属性推論攻撃など、より強い・多様な攻撃による安全性評価。
- **単表以外の形式への適用可能性**: 複数テーブル・時系列データへ DP 合成を拡張する際の前提と限界の整理。

### 7.2 実装・運用上の残課題

以下はエンジニアリング上の課題であり、詳細は [実装ノート](engineering-notes.html) および [環境構築ノート](setup.html) に分離して記載している。

- **環境構築の容易性**: `python-dp` の Windows ホイール非対応など、プラットフォーム制約がある。
- **ライブラリの成熟度**: 研究段階・非公式サポートに由来する実装の粗さ（パッケージング漏れ、機構間の非互換など）。
- **エラー時の切り分け**: 小さい ε での会計破綻など、例外発生時の原因特定。
- **大規模データでの検証**: 本実験は 20,000 行規模であり、より大規模な入力での挙動は未検証。
- **Scalable Pipeline API の評価**: Apache Beam バックエンドでの分散生成は未評価である。
