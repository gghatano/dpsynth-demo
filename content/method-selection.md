...
| 生成時間（本デモ） | 約 10 秒 | 約 90 秒（公式注記では数時間もありうる） | **約 6 秒** |
| 平均 1-way TVD（複数シード）↓ | 0.093 ± 0.019 | 0.109 ± 0.020 | **0.086 ± 0.020** |
| 下流タスク TSTR AUC（複数シード）↑ | **0.670 ± 0.087** | 0.666 ± 0.153 | 0.490 ± 0.090 |
| 2-way 忠実度（実験C）↓ | ペア依存（0.04〜0.36） | **全ペア最良（0.03〜0.15）** | 中位（0.13〜0.20） |
...
- AIM は全ペアで 2-way 忠実度が最良（実験C）。**特定のクロス集計・予測タスクが目的なら AIM が第一候補**。
- MST は木に載ったペアは良好だが、**木から外れたペアは INDEPENDENT より悪化することすらある**（実験C）。
  MST 採用時は、関心のあるペアの 2-way 分布を実データと突き合わせて検証すること。

### 4.2 計算時間・スケール

- 本デモ（9 列・2 万行）で MST 約 10 秒、AIM 約 90 秒。AIM は列数・`max_model_size` を増やすと
  公式注記のとおり**数時間**級になりうる。試行錯誤の回数が必要な段階では MST が現実的。
...
- ε を上げれば分布再現性はおおむね単調に改善する（MST スイープ: ε=0.5 で TVD 0.120 → ε=10 で 0.059、
  [メインレポート §5.1](index.html)）。**機構選定と ε 選定は別の軸**であり、まず ε を要件側
...
- 単一シードでは AIM > MST（AUC 0.768 vs 0.687）に見えたが、複数シードでは差 0.004 と
  **標準偏差の中に消えた**（実験B)。機構間の小さな差で意思決定する場合は、
  必ず複数シードの平均±標準偏差で比較すること。
