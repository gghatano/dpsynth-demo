...
> † **AIM ε=1.0 の相関誤差 0.404 は単一シードの外れ値**である。同じ条件を複数シードで測ると AIM の相関誤差は **0.10 ± 0.16**（5 シード、[実験E](experiments.html)）と分散が極めて大きく、別環境での単一実行では 0.226 や 0.007 も観測された。AIM の相関誤差は点推定では評価できず、§6 の解釈はマルチシードに基づく。

本表から、本実験条件において以下の点が**事実として**読み取れる（定性的判断はいずれも複数シードで裏づけたもの）。

- 平均 1-way TVD は 3 機構とも 0.06〜0.10 の範囲にあり、単一シードでは大小が前後する。複数シードでも 3 機構はほぼ同程度（ε=1 で MST 0.089 / AIM 0.103 / INDEPENDENT 0.091、[実験E](experiments.html)）で、この指標だけでは機構間の差はほとんど判別できない。
...
- MST の平均 1-way TVD は ε の増加に伴っておおむね改善するが、単一シードでは非単調に見える（確率的ばらつきによる）。複数シードでは ε=0.5 → 10 で 0.085 → 0.061 とおおむね低下する（高 ε ほど改善し標準偏差も縮む。[実験E](experiments.html)）。
