...
- **複数シード（[追加実験B](experiments.html)）**: TSTR AUC 0.687 ± 0.068（10 シード）。
  AIM（0.677 ± 0.117）と**ほぼ同等**で、優劣はシード次第で逆転しうる。
...
- **ε スイープ（複数シード・[追加実験E](experiments.html)）**: ε を 0.5 → 10 に上げると平均 1-way TVD は
  0.085 → 0.061 へおおむね改善する（ε=0.5→1.0 は分散内の微増、それ以降は単調低下。標準偏差も ε とともに縮む）。
