...
- **本デモの結論**: UCI Adult Income データ(48,842 行)で実際に生成・評価。
...
```
機微データ ──> 離散化/整数符号化 ──> 1-way マージナル測定(+ノイズ)・ドメイン圧縮
            ──> DP機構(AIM/MST等)が有用なマージナルを選び測定 ──> Private-PGM で同時分布を推定
            ──> 合成レコードをサンプリング ──> 元のスキーマ/型へ復号 ──> DP 合成データ
```
...
**(2) 相関を捨てる INDEPENDENT は下流タスクで破綻。**
INDEPENDENT は 1-way 分布こそ良好(平均 TVD 0.106)だが、**特徴量と収入の相関を一切保たない**ため、
TSTR AUC は **0.433(ランダム以下)・F1 ≈ 0**(全員 `<=50K` と予測)。
これは「**1-way 分布の一致 ≠ 有用性**」「相関のモデル化が下流タスクに不可欠」という DP 合成の核心を実証している。
MST(AUC 0.687)・AIM(AUC 0.768)は相関を捉えることで大きく上回った。
...
[README.md](README.md) を参照(WSL + uv でのセットアップ・実行コマンドを記載)。
